...
TICKER_FILE="/home/dev/stock/intraday_chart_live/input/tickers.csv"
OUTPUT_DIR="/home/dev/stock/intraday_chart_live/output"
CHART_DIR="/home/dev/stock/intraday_chart_live/charts"
...
# ------------------------------------------------------------------
# 6. Live dashboard settings
# ------------------------------------------------------------------
echo
echo
read -p "3. Dashboard columns [default 4]: " chart_cols
chart_cols=${chart_cols:-4}

if ! [[ "$chart_cols" =~ ^[1-9][0-9]*$ ]]; then
    echo "   Invalid columns value. Using default 4."
    chart_cols=4
fi

read -p "   Refresh seconds [default 30]: " refresh_seconds
refresh_seconds=${refresh_seconds:-30}

if ! [[ "$refresh_seconds" =~ ^[1-9][0-9]*$ ]]; then
    echo "   Invalid refresh value. Using default 30."
    refresh_seconds=30
fi
...
echo
echo
echo "4. C++ Fetching live intraday data..."
/home/dev/stock/intraday_chart_live/cpp/bin/fetch_intraday_live
fetch_status=$?
...
if [[ $fetch_status -eq 0 ]]; then
    echo "5. Run Python Script: Generating live dashboard..."
    /home/dev/py/.venv/bin/python /home/dev/stock/intraday_chart_live/py/chart_live.py --live --cols "$chart_cols" --refresh-seconds "$refresh_seconds"
else
