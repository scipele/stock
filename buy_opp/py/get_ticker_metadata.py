...
def load_tickers():
    # Use a set to completely block duplicate entries
    tickers = set()
    with open(TICKER_FILE, "r") as f:
        for row in csv.DictReader(f):
            ticker = row["Ticker"].strip()
            if ticker:
                # Standardize Yahoo Finance format (replace slash with dash)
...
def main():
    ...
    # Filter out missing records using the standardized ticker names
